...
public class PagerSlidingTabStrip extends HorizontalScrollView {
    private List<Integer> listNotice = new ArrayList<Integer>();
    ...
    private boolean IndicatorMatchTab = false;
    ...
    private void addTextTab(final int position, String title) {
        RelativeLayout view = (RelativeLayout) LayoutInflater.from(mContext)
                .inflate(R.layout.tab_top_item, null);
        TextView textViewTitle = (TextView) view
                .findViewById(R.id.tab_item_title);
        TextView textViewNotice = (TextView) view
                .findViewById(R.id.tab_item_notice);
    ...
    private void updateTabStyles() {
        ...
        for (int i = 0; i < tabCount; i++) {
            ...
            if (v instanceof TextView) {
                ...
                TextView textViewTitlle = (TextView) layout.getChildAt(0);
                textViewTitlle.setTextColor(tabTextColor);
                if (i == selectedPosition) {
                    textViewTitlle.setTextColor(selectedTabTextColor);
                    currentPosition = selectedPosition;
                    invalidate();
                }
                TextView textViewNotice = (TextView) layout.getChildAt(1);
                if (listNotice.size() > i && listNotice.get(i) > 0) {
                    textViewNotice.setVisibility(View.VISIBLE);
                    if (listNotice.get(i) > 99) {
                        textViewNotice.setText("99+");
                    } else {
                        textViewNotice.setText(listNotice.get(i) + "");
                    }
                } else {
    ...
    protected void onDraw(Canvas canvas) {
        ...
        RelativeLayout currentTab = (RelativeLayout) tabsContainer
                .getChildAt(currentPosition);
        TextView textViewCurrent = (TextView) currentTab.getChildAt(0);
        float lineLeft, lineRight;

        if (IndicatorMatchTab) {
            lineLeft = currentTab.getLeft() + 10;
            lineRight = currentTab.getRight() - 10;
        } else {
            lineLeft = currentTab.getLeft() + textViewCurrent.getLeft();// textViewCurrent.getLeft()是相对currentTab的左边距离
            lineRight = lineLeft + textViewCurrent.getWidth();
        }

        // if there is an offset, start interpolating left and right coordinates
        // between current and next tab
        if (currentPositionOffset > 0f && currentPosition < tabCount - 1) {
            RelativeLayout nextTab = (RelativeLayout) tabsContainer
                    .getChildAt(currentPosition + 1);
            TextView textViewNext = (TextView) nextTab.getChildAt(0);
            final float nextTabLeft, nextTabRight;
            if (IndicatorMatchTab) {
                nextTabLeft = nextTab.getLeft();
                nextTabRight = nextTab.getRight();
            } else {
                nextTabLeft = nextTab.getLeft()
                        + textViewNext.getLeft();
                nextTabRight = nextTabLeft + textViewNext.getWidth();
            }
    ...
    public void setIndicatorMatchTab(boolean indicatorMatchTab) {
        IndicatorMatchTab = indicatorMatchTab;
    }
